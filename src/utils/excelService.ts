...
export function buildClinicWorkbook(
  ...
  const patientsRows = patients.map((p) => {
    ...
    return {
      ...
      'Email': p.email,
      'Address': p.address,
      'Emergency Contact Person': p.emergencyContactName || (p.emergencyContact?.includes('-') ? p.emergencyContact.split('-')[0].trim() : p.emergencyContact),
      'Emergency Contact Number': p.emergencyContactNumber || (p.emergencyContact?.includes('-') ? p.emergencyContact.split('-')[1].trim() : ''),
      'Emergency Contact Address': p.emergencyContactAddress || '',
      'Emergency Contact (Combined)': p.emergencyContact,
  ...
  patients.forEach((p) => {
    p.checkups.forEach((c) => {
      checkupRows.push({
        ...
        'Net Consultation Fee (₱)': c.fee ? Number(c.fee) : 0,
        'Gross Fee (₱)': c.grossFee ? Number(c.grossFee) : (c.fee ? Number(c.fee) : 0),
        'Discount Type': c.discountType || 'None',
        'Discount Amount (₱)': c.discountAmount ? Number(c.discountAmount) : 0,
        'Payment / Coverage Method': c.paymentMethod || 'Cash',
        'Bank Name': c.bankName || 'N/A',
        'Payment / Transfer Ref #': c.paymentRefNo || 'N/A',
        'HMO Provider': c.hmoProvider || 'N/A',
        'HMO LOA / Approval Code': c.hmoApprovalCode || 'N/A',
        'Senior / PWD ID': c.seniorPwdId || 'N/A',
        'PhilHealth Claim #': c.philhealthClaimNo || 'N/A',
      ...
      try {
        ...
        rawCheckups.forEach((row) => {
          ...
          const record: CheckupRecord = {
            ...
            fee: row['Net Consultation Fee (₱)'] || row['Consultation Fee (₱)'] || row['Fee (₱)'] || row['Fee'] || row['fee'] ? Number(row['Net Consultation Fee (₱)'] || row['Consultation Fee (₱)'] || row['Fee (₱)'] || row['Fee'] || row['fee']) : undefined,
            grossFee: row['Gross Fee (₱)'] ? Number(row['Gross Fee (₱)']) : undefined,
            discountType: row['Discount Type'] && row['Discount Type'] !== 'N/A' ? (row['Discount Type'] as CheckupRecord['discountType']) : undefined,
            discountAmount: row['Discount Amount (₱)'] ? Number(row['Discount Amount (₱)']) : undefined,
            paymentMethod: row['Payment / Coverage Method'] && row['Payment / Coverage Method'] !== 'N/A' ? (row['Payment / Coverage Method'] as CheckupRecord['paymentMethod']) : undefined,
            bankName: row['Bank Name'] && row['Bank Name'] !== 'N/A' ? String(row['Bank Name']) : undefined,
            paymentRefNo: row['Payment / Transfer Ref #'] && row['Payment / Transfer Ref #'] !== 'N/A' ? String(row['Payment / Transfer Ref #']) : (row['Reference #'] ? String(row['Reference #']) : undefined),
            hmoProvider: row['HMO Provider'] && row['HMO Provider'] !== 'N/A' ? String(row['HMO Provider']) : undefined,
            hmoApprovalCode: row['HMO LOA / Approval Code'] && row['HMO LOA / Approval Code'] !== 'N/A' ? String(row['HMO LOA / Approval Code']) : undefined,
            seniorPwdId: row['Senior / PWD ID'] && row['Senior / PWD ID'] !== 'N/A' ? String(row['Senior / PWD ID']) : undefined,
            philhealthClaimNo: row['PhilHealth Claim #'] && row['PhilHealth Claim #'] !== 'N/A' ? String(row['PhilHealth Claim #']) : undefined,
        ...
        const parsedPatients: Patient[] = rawPatients.map((row, index) => {
          ...
          return {
            ...
            emergencyContact: String(row['Emergency Contact (Combined)'] || row['Emergency Contact'] || row['emergencyContact'] || 'N/A'),
            emergencyContactName: row['Emergency Contact Person'] ? String(row['Emergency Contact Person']) : undefined,
            emergencyContactNumber: row['Emergency Contact Number'] ? String(row['Emergency Contact Number']) : undefined,
            emergencyContactAddress: row['Emergency Contact Address'] ? String(row['Emergency Contact Address']) : undefined,
